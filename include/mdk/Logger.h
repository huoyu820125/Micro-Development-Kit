...
public:
	...
	void Info( const char *findKey, const char *format, ... );//输出信息，输出格式为 时间+空格+(findKey)+空格+(Tid:线程ID)+空格+参数传入信息
	void StreamInfo( const char *findKey, unsigned char *stream, int nLen, const char *format, ...  );//输出带数据流的日至信息, stream数据将接在format信息之后输出，以stream:为标记
	void SetLogName( const char *name );//设置日志命，日志将被保存到"log/名字/"下目录下
